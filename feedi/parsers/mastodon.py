...
def register_app(server_url, callback_url):
    return mastodon.Mastodon.create_app(
        CLIENT_NAME, api_base_url=server_url, redirect_uris=[callback_url], scopes=SCOPES
    )


def auth_redirect_url(server_url, client_id, client_secret, callback_url):
    client = mastodon.Mastodon(client_id=client_id, client_secret=client_secret, api_base_url=server_url)
    return client.auth_request_url(client_id=client_id, scopes=SCOPES, redirect_uris=callback_url)
...
def fetch_account_data(server_url, access_token):
    client = mastodon.Mastodon(access_token=access_token, api_base_url=server_url)
    return client.me()
...
def fetch_toots(server_url, access_token, newer_than=None, limit=None):
    ...
    entries = []
    for toot in toots:
        entry = {"raw_data": json.dumps(toot, default=str)}
